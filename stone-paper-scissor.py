...
def again():
    ...
    while(round != 4):
        ...
        user = int(input("Stone-Paper-Scissor 1-2-3: "))
        com = key, val = random.choice(list(element.items()))
        print("computer choose.. :", com, com[0], "<->", user)
        if (user == 1 and int(com[0]) == 3) or (user == 2 and int(com[0]) == 1) or (user == 3 and int(com[0]) == 2):
            i = i+1
